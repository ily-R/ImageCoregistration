...
def getClicksAndDiscriptor(target, source, target_gray, source_gray):
    ...
    return pts1_all[:p1.shape[0]], pts2_all[:p1.shape[0]], des1[:p1.shape[0]], des2[:p1.shape[0]]


def getKeypointAndDiscriptors(target_gray, source_gray):
    sift = cv2.xfeatures2d.SIFT_create()
    kp1, des1 = sift.detectAndCompute(target_gray, None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ransac", help="apply ransac", action="store_true")
    ...
    if not args.sift:
        on_target = True
        p1, p2 = [], []
        lmk1, lmk2, desc1, desc2 = getClicksAndDiscriptor(target, source, target_gray, source_gray)
    else:
        lmk1, lmk2, desc1, desc2 = getKeypointAndDiscriptors(target_gray, source_gray)

    lmk1, lmk2 = match(lmk1, lmk2, desc1, desc2)
    display_matches(target, source, lmk1, lmk2)
    if args.ransac:
        lmk1, lmk2 = ransac(lmk1, lmk2)

    T = calculate_transform(lmk2, lmk1)
    w = warp(target, source, T)